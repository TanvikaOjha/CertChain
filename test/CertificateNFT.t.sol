...
contract CertificateNFTTest is Test {
    ...
    function _certOf(uint256 tokenId) internal view returns (CertificateNFT.Certificate memory) {
        (
            address _student, address _issuer, string memory _course,
            string memory _degree, string memory _institution,
            uint256 _issue, uint256 _expiry, bool _revoked,
            string memory _reason, string memory _ipfs
        ) = cert.certificates(tokenId);
        return CertificateNFT.Certificate({
            student: _student, issuer: _issuer,
            courseName: _course, degree: _degree,
            institutionName: _institution,
            issueDate: _issue, expiryDate: _expiry,
            isRevoked: _revoked, revokeReason: _reason,
            ipfsHash: _ipfs
        });
    }
    ...
    function test_Integration_ThreeIssuersParallelSessions() public {
        ...
        vm.prank(college); uint256 ca = cert.issueCertificate(student,  COURSE_A, DEGREE_UG, 0, IPFS_A);
        vm.prank(mit);     uint256 cb = cert.issueCertificate(student2, COURSE_B, DEGREE_PG, 0, IPFS_B);
        vm.prank(iit);     uint256 cc = cert.issueCertificate(student3, "ML", DEGREE_DR,     0, IPFS_C);
    ...
    function testFuzz_ExpiryBoundaryAlwaysCorrect(
