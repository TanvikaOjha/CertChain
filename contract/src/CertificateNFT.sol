...
contract CertificateNFT is ERC721URIStorage, Ownable {
    ...
    function revokeCertificate(uint256 tokenId, string memory reason) external onlyIssuer certExists(tokenId) {
        require(certificates[tokenId].issuer == msg.sender, "Only original issuer can revoke");
    ...
    function verifyCertificate(uint256 tokenId)
        ...
        view
        certExists(tokenId)
        returns (bool isValid, string memory status, Certificate memory cert)
    {
    ...
    function getCertsByStudent(address _student) external view returns (uint256[] memory) {
